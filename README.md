...
on the root directory, install the project packages with npm install using node version 20 or higher
change directory to backend to create databse tables by running npx sequelize-cli db:migrate to run project migrations with sequelize
then change back to root directory and run the sever npm run server
